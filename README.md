...
- **Challenge:** Merging high-frequency asynchronous trade executions with daily sentiment metrics without losing critical granularity or introducing look-ahead bias.
  - **Solution:** Implemented robust Pandas `merge_asof` and custom grouping logic to accurately attribute trades to the exact prevailing sentiment at execution time. This highlighted the necessity of advanced data alignment techniques in time-series analysis.
- **Challenge:** Handling outliers and extreme anomalies in historical trade data (e.g., liquidation spikes).
  - **Solution:** Applied IQR filtering and custom robust statistical techniques to isolate and manage anomalies, ensuring the final analysis reflected genuine behavioral trends rather than noise.
- **Takeaway:** Building this pipeline reinforced the importance of writing scalable data-cleaning functions, maintaining a deep understanding of index alignment in time-series analysis, and having a strong engineering mindset when solving complex data challenges.
